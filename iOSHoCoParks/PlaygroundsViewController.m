//
//  PlaygroundsViewController.m
//  iOSHoCoParks
//
//  Created by Yongyuth Phasukyued on 6/17/15.
...
@interface PlaygroundsViewController () {
    UIView *mainView;
    UIView *mainMapView;
    UIView *infoView;
...
@implementation PlaygroundsViewController
...
-(void)openGoogleNavigationEntrance:(NSString *)mode ly:(CGFloat)latY lx:(CGFloat)lngX {
    NSInteger wifiAvailable = [PlaygroundsViewController checkAvailableWiFi];
    NSInteger netAvailable = [PlaygroundsViewController checkAvailableInternet];
    if (wifiAvailable == 1) {
...
- (void)information_Click:(CGFloat)page {
    if (page == 3) {
        ...
        NSString* jsonFilePath = [[NSBundle mainBundle] pathForResource:[@"dataFeatureDescription" stringByReplacingOccurrencesOfString:@" / " withString:@" "] ofType:@"json"];
        ...
        for(int i=0; i< [json count]; i++) {
            NSDictionary *loc = [json objectAtIndex:i];
            if ([[loc objectForKey:@"name"] isEqualToString:@"Playgrounds"]) {
                info_TextView.text = [NSString stringWithFormat:@"In the emergency call 911\nPlayground ID: %@\n\n%@", self.feature_id, [loc objectForKey:@"description"]];
            }
        }
    }
}
...
- (void) pageTurn: (UIPageControl *) aPageControl {
    ...
    if (aPageControl.currentPage == 0) {
        ...
        appSubTitle.text = self.site_name;
...
- (void)viewDidLoad {
    ...
    UIImage *myimage = [UIImage imageNamed:[NSString stringWithFormat:@"%@.jpg", self.feature_id]];
    ...
    CIImage *inputImage1 = [CIImage imageWithCGImage:[[UIImage imageNamed:[NSString stringWithFormat:@"%@.jpg", self.feature_id]] CGImage]];
    ...
    */
    ...
    //self.parks = [GetJSON getNearest:@"All" searchItem:@"Playgrounds" mv:self.mapView];
    //[self removeAllAnnotations];
    //[self setPin];
...
-(void)back_Click {
    ...
    if ([self.backViewName isEqualToString:@"NearestResult"]) {
        ...
        vc.searchTitle = self.searchItem;
        vc.park_id = self.park_id;
        vc.latItem = self.latItem;
        vc.lngItem = self.lngItem;
        vc.backViewName = @"Main";
