...
@interface PavilionsViewController () {
    UIView *mainView;
    UIView *mainMapView;
    UIView *infoView;
...
@synthesize searchSelected, searchItem, feature_id, latItem, lngItem, site_name, backViewName, park_id;
@synthesize searchType, trailID, hasSavedLocation, latSaved, lngSaved;

-(void)checkWalking {
    [indicator startAnimating];
    [self startLocationUpdates];
    timer = [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(countParkingDriving) userInfo:nil repeats:NO];
}

-(void)countParkingWalking {
    NSMutableArray *json;
    json = [GetJSON countFeatureByParkID:self.park_id];
    for(int i=0; i< [json count]; i++) {
        NSDictionary *loc = [json objectAtIndex:i];
        
        if ([[loc objectForKey:@"PARKING"]integerValue] == 0) {
            [self openGoogleNavigationEntrance:@"walking" ly:latItem lx:lngItem];
        } else if ([[loc objectForKey:@"PARKING"]integerValue] >= 1) {
            [self getNearestParking:@"walking"];
        }
    }
}
...
-(void)countParkingDriving {
    ...
    for(int i=0; i< [json count]; i++) {
        ...
        if ([[loc objectForKey:@"PARKING"]integerValue] == 0) {
            [self openGoogleNavigationEntrance:@"driving" ly:latItem lx:lngItem];
        } else if ([[loc objectForKey:@"PARKING"]integerValue] >= 1) {
            [self getNearestParking:@"driving"];
        }
    }
}

-(void)getNearestParking:(NSString *)mode {
    NSMutableArray *json;
    json = [GetJSON getFeatureByParkID:self.park_id searchItem:@"All_Amenities" mv:self.mapView];
    
    NSArray *parking;
    NSMutableArray *parkings = [[NSMutableArray alloc] init];
    
    for(int i=0; i< [json count]; i++) {
        NSDictionary *loc = [json objectAtIndex:i];
        if ([[loc objectForKey:@"feature_type"] isEqualToString:@"PARKING"]) {
            ...
            parking =[[NSArray alloc]initWithObjects:dict, nil];
            [parkings addObjectsFromArray:parking];
        }
    }
    ...
    for(int j=0; j< [parkings count]; j++) {
        NSDictionary *loc1 = [parkings objectAtIndex:j];
        CLLocation *parkingLocation = [[CLLocation alloc]
                                   initWithLatitude:[[loc1 objectForKey:@"latitude"]floatValue]
                                   longitude:[[loc1 objectForKey:@"longitude"]floatValue]];
        
        CLLocation *pavilionLocation = [[CLLocation alloc]
                                    initWithLatitude:latItem
                                    longitude:lngItem];

        CLLocationDistance dist = [pavilionLocation distanceFromLocation:parkingLocation];
        NSString *parkingDistance = [NSString stringWithFormat:@"%.02f", dist/1609.34];
        
        NSDictionary * dict =[NSMutableDictionary new];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"feature_name"]] forKey:@"feature_name"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"latitude"]] forKey:@"latitude"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"longitude"]] forKey:@"longitude"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"id"]] forKey:@"id"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"park_id"]] forKey:@"park_id"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"site_name"]] forKey:@"site_name"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"feature_type"]] forKey:@"feature_type"];
        [dict setValue:[NSString stringWithFormat:@"%@",[loc1 objectForKey:@"feature_id"]] forKey:@"feature_id"];
        [dict setValue:[NSNumber numberWithFloat:[[NSString stringWithFormat:@"%@", parkingDistance]floatValue]] forKey:@"distance"];
    ...
    for(int i=0; i< [distances count]; i++) {
        NSDictionary *loc = [distances objectAtIndex:i];
        float currentValue = [[loc objectForKey:@"distance"]floatValue];
    ...
    for(int i=0; i< [distances count]; i++) {
        ...
        if (currentValue < minDist) {
            ...
            parkingY = [[loc objectForKey:@"latitude"]floatValue];
            parkingX = [[loc objectForKey:@"longitude"]floatValue];
        }
    }
    [self openGoogleNavigationEntrance:mode ly:parkingY lx:parkingX];
}
-(void)openGoogleNavigationEntrance:(NSString *)mode ly:(CGFloat)latY lx:(CGFloat)lngX {
    NSInteger wifiAvailable = [PavilionsViewController checkAvailableWiFi];
    NSInteger netAvailable = [PavilionsViewController checkAvailableInternet];
    if (wifiAvailable == 1) {
...
- (void)information_Click:(CGFloat)page {
    info_TextView.text = @"";
    if (page == 0) {
        if ([feature_id isEqualToString:@"P025"]
            || [feature_id isEqualToString:@"P029"]
            || [feature_id isEqualToString:@"P035"]
            || [feature_id isEqualToString:@"P037"]
            || [feature_id isEqualToString:@"P038"]
            ) {
            info_TextView.text = @"This pavilion is first-come, first-served and cannot be rented.";
        } else {
            info_TextView.text = @"Blackout Dates\n\nDue to the high volume of requests and special events in our Parks,the following dates and locations are not available for rentals:\n\nCentennial Park- Pavilions A, B and C\n\n  Saturday, April 11\n  Sunday, April 18\n  Saturday,  April 25\n  Saturday, May 9\n  Saturday and Sunday, May 17 & 18\n  Saturday, June 6\n  Sunday, June 7\n  Sunday, August 9\n  Saturday and Sunday, August 16 & 17\n  Sunday, September 13\n  Saturday, September 19\n  Saturday, September 26\n  Sunday, October 4\n  Saturday, October 24\n  Saturday, November 21\n  Saturday, November 28\n  Monday – Friday June 15 through August 31\n\nCentennial Park Pavilions E, F and G\n\n  Monday- Friday, June 15 through August 31\n\nCentennial Park Pavilion O (Overlook Pavilion)\n\n  Saturdays and Sundays – April through July\n\nPlease Note:\n\nCentennial Park Pavilion D and H have verylimited weekend availability between May and September.";
        }
        ...
        NSString* jsonFilePath = [[NSBundle mainBundle] pathForResource:[@"dataPavilionsInformation" stringByReplacingOccurrencesOfString:@" / " withString:@" "] ofType:@"json"];
        ...
        for(int i=0; i< [json count]; i++) {
            NSDictionary *loc = [json objectAtIndex:i];
            if ([feature_id isEqualToString:[loc objectForKey:@"Ident"]]) {
                NSString *description = [loc objectForKey:@"Description"];
                if ([[loc objectForKey:@"Ident"] isEqualToString:@"P025"]
                    || [[loc objectForKey:@"Ident"] isEqualToString:@"P029"]
                    || [[loc objectForKey:@"Ident"] isEqualToString:@"P035"]
                    || [[loc objectForKey:@"Ident"] isEqualToString:@"P037"]
                    || [[loc objectForKey:@"Ident"] isEqualToString:@"P038"]
                    ) {
                    info_TextView.text = [NSString stringWithFormat:@"In the emergency call 911\nPavilion ID: %@\n\n%@ - %@\n%@", self.feature_id,self.site_name,searchSelected,description];
                } else {
                    NSString *capacity = [loc objectForKey:@"Capacity"];
                    NSString *residents_fee = [loc objectForKey:@"Residents_Fee"];
                    NSString *non_residents_fee = [loc objectForKey:@"Non_Residents_Fee"];
                    NSString *amenities_included = [[loc objectForKey:@"Amenities_Included"]stringByReplacingOccurrencesOfString:@"\\\\n" withString:@"\n - "];
                    NSString *amenities_not_included = [[loc objectForKey:@"Amenities_Not_Included"]stringByReplacingOccurrencesOfString:@"\\\\n" withString:@"\n - "];
                    
                    info_TextView.text = [NSString stringWithFormat:@"In the emergency call 911\nPavilion ID: %@\n\n%@ - %@\n%@\n\nFee (each):\n$ %@/day(residents)\n$ %@/day(non-residents)\n\nMaximum Capacity: %@\n\nAmenities(Included with permit)\n - %@\n\nAdditional amenities and facilities located nearby this pavilion(not included with permit)\n - %@", self.feature_id,self.site_name,searchSelected,description,residents_fee,non_residents_fee,capacity,amenities_included,amenities_not_included];
                }
        ...
        if ([feature_id isEqualToString:@"P025"]
            ...
            info_TextView.text = @"Rental Requests\n\nPavilions can be rented up to one year to the day in advance. Reservation applicants must be at least 21 years old. The person who signs the permit is responsible for the guests who come with him or her to the park.\n\nMost pavilions are available from April 1 through November 30, rain or shine; no refunds for inclement weather. Rental may begin as early as 10 AM and must conclude by dusk. You MUST leave the park by the designated time of dusk or be charged a late fee of $50 per ½ hour. A clean-up fee may be assessed if necessary; see Trash Pick-up on page 47. All fees are non-refundable.\n\nYour reservation will be canceled if we do not receive your signed permit with payment by the due date shown on your contract. The application must be signed and dated to be valid; unsigned applications will be returned. Make check or money order payable to the Director of Finance Howard County.\n\nCall 410-313-4682 if you have not received an application within 24 hours of your request. All applications indicate the applicable fees and a payment due date. Requests made less than two weeks from date of use must be completed within one hour of request. Weekly pavilion rental schedule is posted in the pavilion each week. If someone is using the pavilion you reserved at the time of your arrival, show them a copy of your approved permit; if necessary, call the park staff phone number provided on your permit or 410-313-2929 to page the on-call supervisor. Be sure to take your permit with you.\n\nFor reservations, call 410-313-4682. Have the following information readily available.\n\n  - Which pavilion you wish to rent.\n  - Name, address, telephone number, e-mail address and date of birth of responsible party.\n  - Maximum attendance - indicate the maximum number of people expected. Attendance cannot exceed the capacity of the pavilion.\n  - Time of use - pavilions can be rented no earlier than 10 AM and no later than dusk.\n  - Moonbounce - Proof of at least $1,000,000 general liability insurance; $1,000,000 per occurrence. Howard County government must be listed as the Certificate Holder. Name, address and telephone contact information for the rental company needs to be listed on the moonbounce disclaimer. Additionally, physical site limitations may prohibit use of certain equipment. The certificate of insurance from the rental company must accompany the signed moonbounce permit.\n\nFields and ball diamonds requiring permits for use are available to rent on a very limited basis after they have been allocated for leagues and programs. Call 410-313-4626 for more information.";
            
        }

    } else if (page == 4) {
        info_TextView.text = @"Policies\n\nPlease read the guidelines and review with your group or family. These guidelines are in effect for use of all Howard County Recreation & Parks’ pavilions. If you have any questions or concerns, please call 410-313-4682.\n\nThe Department of Recreation & Parks is pleased to comply with the American with Disabilities Act (ADA). Applicants and their guests must comply with all ADA regulations. Applicants and their guests agree to follow and enforce the policies included in this document and the “Rules and Regulations for Howard County Parks” as abbreviated from Section 19.200 through 19.211 of the Howard County Code. See the County Code for penalties for violations of these sections.\n\nSection 24.107 of the County Code states that any person who violates the provisions of this Subtitle shall, upon conviction, be found guilty of a misdemeanor and may be fined up to $1,000 or imprisoned.\n\nThe Department reserves the right to deny or cancel any pavilion agreement for any reasonable cause or violation of this agreement. Every attempt will be made to give the applicant adequate notice of cancellation or request denial.\n\nRules of Conduct\n\n  - The applicant must be present at all times. That individual is responsible for damage to property and supervision and control of all guests while using the facility.\n  - Alcoholic beverages are prohibited outside the pavilion. Glass containers are prohibited.\n  - Howard County Recreation & Parks enforces the noise level restrictions established by the Howard County Police Department.\n  - Use of charcoal grills (up to 22.5”) and gas grills (up to four burners) are allowed. Park staff and Rangers have the right to inspect grills. They can request to relocate the grill or to remove the grill. Propane burners and fryers are strictly prohibited.\n  - Smoking or the use of tobacco products or any kind is prohibited in all parks, pavilions, playgrounds, restrooms, sports areas and at concerts and special events.\n  - Soliciting or loitering is prohibited.\n  - Interference with employee and volunteer duties is prohibited by law.\n  - Inappropriate or indecent conduct, harassment and/or language is strictly prohibited.\n  - Use of illegal drugs and weapons is strictly prohibited by law.\n  - Parking or driving on turf or unauthorized areas is prohibited.\n  - Alteration or installation of equipment (such as volleyball nets) is prohibited.\n  - Defacing of property (indoors or outdoors) is prohibited. (No staples, nails or signs.)\n  - Any other conduct that may jeopardize the safety of others is prohibited.\n  - Full compliance with Rules and Regulations for Howard County Parks is required.\n  - Full compliance with animal control laws is required. All leashed pets are allowed in the park unless otherwise posted. Howard County law requires owners to clean up after their pets.\n  - No golfing.";
    }
}
- (void)map_Click {
    ...
    NSString *titleCode = [NSString stringWithFormat:@"AMENITIES-%@", self.searchSelected];
...
- (void) pageTurn: (UIPageControl *) aPageControl {
    ...
    if (aPageControl.currentPage == 0) {
        whichPage = 0;
        pageControl.frame = CGRectMake(40, myScrollView.frame.size.height-20, self.view.frame.size.width, 10);
        ...
        [self information_Click:0];
    } else if (aPageControl.currentPage == 1) {
        [self postTipMessage:@"For your current location and compass." type:@"current"];
        whichPage = 60;
        pageControl.frame = CGRectMake(20, myScrollView.frame.size.height-20, self.view.frame.size.width, 10);
...
- (void)viewDidLoad {
    ...
    UIImage *myimage = [UIImage imageNamed:[NSString stringWithFormat:@"%@.jpg", feature_id]];
    myimage = [UIImage imageWithCGImage:myimage.CGImage scale:myimage.scale orientation:UIImageOrientationRight];
    
    CIImage *inputImage = [CIImage imageWithCGImage:[[UIImage imageNamed:@"Entry_Background.jpg"] CGImage]];
    CIImage *inputImage1 = [CIImage imageWithCGImage:[[UIImage imageNamed:[NSString stringWithFormat:@"%@.jpg", feature_id]] CGImage]];
    ...
    mainImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, mainView.frame.size.width, mainView.frame.size.height)];
    ...
    */
    ...
    pageControl.frame = CGRectMake(40, myScrollView.frame.size.height-10, self.view.frame.size.width, 10);
    ...
    [page1Title setTitle:@"Available" forState:UIControlStateNormal];
    page1Title.tag = 0;
    ...
    UIView *myView2 = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)+40, myScrollView.frame.size.height-45, 40, 40)];
    myView2.backgroundColor = [UIColor clearColor];
    [myScrollView addSubview:myView2];
    
    page2Title = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 30)];
    [page2Title addTarget:self action:@selector(initPageturn:) forControlEvents:UIControlEventTouchUpInside];
    [page2Title setTitle:@"Map" forState:UIControlStateNormal];
    page2Title.tag = 1;
    ...
    UIView *myView3 = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)+80, myScrollView.frame.size.height-45, 80, 40)];
    myView3.backgroundColor = [UIColor clearColor];
    [myScrollView addSubview:myView3];
    
    page3Title = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 80, 30)];
    [page3Title addTarget:self action:@selector(initPageturn:) forControlEvents:UIControlEventTouchUpInside];
    [page3Title setTitle:@"Overview" forState:UIControlStateNormal];
    page3Title.tag = 2;
    ...
    UIView *myView4 = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)+160, myScrollView.frame.size.height-45, 60, 40)];
    myView4.backgroundColor = [UIColor clearColor];
    [myScrollView addSubview:myView4];
    
    page4Title = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 60, 30)];
    [page4Title addTarget:self action:@selector(initPageturn:) forControlEvents:UIControlEventTouchUpInside];
    [page4Title setTitle:@"Rental" forState:UIControlStateNormal];
    page4Title.tag = 3;
    ...
    UIView *myView5 = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)+220, myScrollView.frame.size.height-45, 70, 40)];
    myView5.backgroundColor = [UIColor clearColor];
    [myScrollView addSubview:myView5];
    
    page5Title = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 70, 30)];
    [page5Title addTarget:self action:@selector(initPageturn:) forControlEvents:UIControlEventTouchUpInside];
    [page5Title setTitle:@"Policies" forState:UIControlStateNormal];
    page5Title.tag = 4;
    ...
    appSubTitle.text = [NSString stringWithFormat:@"%@", self.site_name];
    ...
    [menuDriveBTN addTarget:self action:@selector(checkParking) forControlEvents:UIControlEventAllEvents];
    ...
    //self.parks = [GetJSON getNearest:(NSString *)@"All" searchItem:(NSString *)@"Pavilions" mv:self.mapView];
    //[self removeAllAnnotations];
    //[self setPin];
...
-(void)openCategory {
    ...
    NSData *jsonData = [NSData dataWithContentsOfFile:jsonFilePath options:kNilOptions error:&jsonError ];
    cells = [NSJSONSerialization JSONObjectWithData:jsonData options:kNilOptions error:&error];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary *info;
    info = [cells objectAtIndex:indexPath.row];
...
- (void)changeAmenities:(id)sender{
    if([sender isOn]) {
        ...
        [self removeAllAnnotations];
        [self.amenities removeAllObjects];
        self.amenities = [GetJSON getFeatureByParkID:(NSString *)@"All" searchItem:(NSString *)@"Some_Amenities" mv:(MKMapView *)self.mapView];
        searchItem = @"Amenities";
        [self setPin];
    } else {
...
-(void)openProfile:(NSMutableArray *)myprofiles {
    ...
    [distances1 removeAllObjects];
    distances1 = [GetJSON getDistanceProfile:myprofiles];
    
    float maxDist = 0.0;
    for(int i=0; i< [distances1 count]; i++) {
        NSDictionary *loc = [distances1 objectAtIndex:i];
