//
//  HistoricSitesViewController.m
//  iOSHoCoParks
//
//  Created by Yongyuth Phasukyued on 6/18/15.
...
@interface HistoricSitesViewController () {
    UIView *mainView;
    UIView *mainMapView;
    UIView *infoView;
...
@implementation HistoricSitesViewController
@synthesize searchSelected, searchItem, feature_id, latItem, lngItem, backViewName, park_id, site_name;
...
-(void)countParkingWalking {
    ...
    //if ([searchItem isEqualToString:@"Historic Sites"]) {
...
-(void)openGoogleNavigationEntrance:(NSString *)mode ly:(CGFloat)latY lx:(CGFloat)lngX {
    NSInteger wifiAvailable = [HistoricSitesViewController checkAvailableWiFi];
    NSInteger netAvailable = [HistoricSitesViewController checkAvailableInternet];
    if (wifiAvailable == 1) {
...
- (void)information_Click {
    ...
    json0 = [GetJSON getNearest:@"Nearest" searchItem:searchItem mv:self.mapView];
    
    NSString *address, *capitalizedAddress, *park_description, *finalAddress;
    for(int k=0; k< [json0 count]; k++) {
        ...
        if ([searchSelected isEqualToString:[loc0 objectForKey:@"feature_name"]]) {
            ...
            if ([site_name isEqualToString:@"CENTENNIAL PARK"]) {
            ...
            } else if ([site_name isEqualToString:@"CEDAR LANE PARK"]) {
                address = [NSString stringWithFormat:@"%@ (West)\n10745 Route 108 (East)\n%@", [loc0 objectForKey:@"address1"],[loc0 objectForKey:@"address2"]];
            } else if ([site_name isEqualToString:@"ROCKBURN BRANCH PARK"]) {
    ...
    info_TextView.text = [NSString stringWithFormat:@"%@\n\n%@\n\n%@", site_name, finalAddress, park_description];
    
    if ([site_name isEqualToString:@"Belmont Manor and Historic Park"]) {
        UIButton *urlBTN = [[UIButton alloc] initWithFrame:CGRectMake(10, 10, 75, 30)];
        [urlBTN addTarget:self action:@selector(openBelmont) forControlEvents:UIControlEventTouchUpInside];
        [urlBTN setTitle:@"Web Site" forState:UIControlStateNormal];
        urlBTN.backgroundColor = [UIColor grayColor];
        urlBTN.titleLabel.font = [UIFont systemFontOfSize:14];
        urlBTN.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        [infoView addSubview:urlBTN];
    }
}
- (void)map_Click {
...
- (void)checkMapSetting {
    if([amenitiesDisplay isEqualToString:@"ON"]) {
        ...
        [self removeAllAnnotations];
        searchItem = @"Historic Sites";
        [self.parks removeAllObjects];
        self.parks = [GetJSON getNearest:@"All" searchItem:@"Historic Sites" mv:self.mapView];
        [self setPin];
    }
}
...
- (void)viewDidLoad {
    ...
    */
    ...
    pageControl.frame = CGRectMake(10, myScrollView.frame.size.height-10, self.view.frame.size.width, 10);
    ...
    UIView *myView1 = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)-20, myScrollView.frame.size.height-45, 40, 40)];
    myView1.backgroundColor = [UIColor clearColor];
    [myScrollView addSubview:myView1];
    
    page1Title = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 30)];
    [page1Title addTarget:self action:@selector(initPageturn:) forControlEvents:UIControlEventTouchUpInside];
    [page1Title setTitle:@"Map" forState:UIControlStateNormal];
    page1Title.tag = 0;
    ...
    UIView *myView2 = [[UIView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)+20, myScrollView.frame.size.height-45, 80, 40)];
    myView2.backgroundColor = [UIColor clearColor];
    [myScrollView addSubview:myView2];
    
    page2Title = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 80, 30)];
    [page2Title addTarget:self action:@selector(initPageturn:) forControlEvents:UIControlEventTouchUpInside];
    [page2Title setTitle:@"Overview" forState:UIControlStateNormal];
    page2Title.tag = 1;
...
-(void)back_Click {
    ...
    if ([self.backViewName isEqualToString:@"NearestResult"]) {
        ...
        vc.searchItem = self.searchSelected;
        vc.searchSelected = self.searchSelected;
        vc.park_id = self.park_id;
        vc.site_name = self.site_name;
        vc.feature_id = self.feature_id;
